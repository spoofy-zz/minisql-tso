...
SELECT * FROM PEOPLE WHERE GRAD='ZAGREB';
SELECT * FROM PEOPLE WHERE ID BETWEEN 1 AND 3;
SELECT * FROM PEOPLE WHERE IME LIKE 'A%' OR GRAD='RIJEKA';
UPDATE PEOPLE SET GRAD='RIJEKA' WHERE ID=1;
...
SELECT * FROM PEOPLE;
SELECT * FROM PEOPLE WHERE GRAD='ZAGREB';
SELECT * FROM PEOPLE WHERE ID > 0 AND IME LIKE 'A%';
.QUIT
/*
```
...
- creates secondary index `IDXGRAD` on column `GRAD`
- prints `.SCHEMA PEOPLE`
- tests `SELECT * FROM PEOPLE`
- tests `SELECT * FROM PEOPLE WHERE GRAD='RIJEKA'`
...
```text
OK TABLE CREATED
PEOPLE(ID INT PRIMARY KEY, IME VARCHAR(8), GRAD CHAR(8))
ERR BAD INT VALUE
ERR VALUE TOO LONG
OK INDEX CREATED
INDEX IDXGRAD ON PEOPLE(GRAD)
ID | IME | GRAD
2 | IVO | RIJEKA
OK 1 ROWS
```
...
SELECT * FROM PEOPLE WHERE ID > 1 AND GRAD LIKE 'RI%';
SELECT * FROM PEOPLE WHERE ID BETWEEN 1 AND 3;
SELECT * FROM PEOPLE WHERE IME LIKE 'P%' OR GRAD='SPLIT';
SELECT * FROM PEOPLE WHERE GRAD='RIJEKA';
UPDATE PEOPLE SET GRAD='SISAK' WHERE ID=3;
SELECT * FROM PEOPLE WHERE GRAD='RIJEKA';
SELECT * FROM PEOPLE WHERE GRAD='SISAK';
...
- `.SCHEMA PEOPLE` shows `ID PRIMARY KEY` and `INDEX IDXGRAD ON PEOPLE(GRAD)`.
- `SELECT ... WHERE GRAD='RIJEKA'` uses the secondary index when available.
- `WHERE` supports `AND`, `OR`, `LIKE`, `<`, `>` and `BETWEEN`.
- `ID` accepts only whole numbers because it is `INT`.
- `IME` accepts at most 8 characters because it is `VARCHAR(8)`.
- duplicate `ID=3` returns `ERR DUPLICATE PRIMARY KEY`.
- changing a primary key column returns `ERR CANNOT UPDATE PRIMARY KEY`.
- changing a non-key column such as `GRAD` works and rebuilds indexes.
